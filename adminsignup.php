...
{
    header("location:adminsignup.php?msg=3");
}
    else{
            $select = "SELECT * FROM `admin` WHERE email='$email'";
            $result = $conn->query($select);
            if (mysqli_num_rows($result) > 0) {
                header("location:adminsignup.php?msg=0");
            } 
            else {
                $sql = "INSERT INTO admin ". "(email,name, contact, 
               address,password) ". "VALUES('$email','$name',$contact,'$address','$password')";
                if ($conn->query($sql) ) {
                    header("location:adminsignup.php?msg=1");
                }
            }
    }
}
...
<div class="container">
    ...
    <div class="middle">
        <div class="headings">ADMIN SIGNUP</div>
        <form action="adminsignup.php" method="POST" enctype="multipart/form-data" id="myform">
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="password" name="repassword" data-rule-required="true"
                       data-msg-required="Please Confirm Password">
            </div>

            <div class="form-group">
                <button type="submit" name="signup" id="signup" class="btn btn-success">Submit</button>
            </div>    
            Already have an account?<a href="adminlogin.php" style="text-decoration:none; font-family:'Play',sans-serif;color:blue;">&nbsp;Log In</a>
